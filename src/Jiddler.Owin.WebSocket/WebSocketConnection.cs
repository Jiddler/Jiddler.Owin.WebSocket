...
using Jiddler.Owin.WebSocket.Handlers;
...
namespace Jiddler.Owin.WebSocket {
    public abstract class WebSocketConnection {
        ...
        /// </summary>
        public TaskQueue QueueSend => _webSocket.SendQueue;
        ...
        public Task Close(WebSocketCloseStatus status, string reason) {
            return _webSocket.Close(status, reason, CancellationToken.None);
        }
        ...
        public Task SendBinary(ArraySegment<byte> buffer, bool endOfMessage) {
            return _webSocket.SendBinary(buffer, endOfMessage, _cancelToken.Token);
        }
        ...
        public Task SendText(ArraySegment<byte> buffer, bool endOfMessage) {
            return _webSocket.SendText(buffer, endOfMessage, _cancelToken.Token);
        }
        ...
        public Task Send(ArraySegment<byte> buffer, bool endOfMessage, WebSocketMessageType type) {
            return _webSocket.Send(buffer, type, endOfMessage, _cancelToken.Token);
        }
        ...
        private async Task RunWebSocket(IDictionary<string, object> websocketContext) {
            ...
            if (websocketContext.TryGetValue(typeof(WebSocketContext).FullName, out value)) {
                _webSocket = new NetWebSocket(((WebSocketContext) value).WebSocket);
            } else {
            ...
            do {
                try {
                    received = await _webSocket.ReceiveMessage(buffer, _cancelToken.Token);
            ...
            try {
                await _webSocket.Close(WebSocketCloseStatus.NormalClosure, string.Empty, _cancelToken.Token);
            } catch {
                //Ignore
            }
            ...
            OnClose(_webSocket.CloseStatus, _webSocket.CloseStatusDescription);
            await OnCloseAsync(_webSocket.CloseStatus, _webSocket.CloseStatusDescription);
        }
